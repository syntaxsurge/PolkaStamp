...
export function WalletSelect({
  ...
  useEffect(() => {
    if (
      ...
      selectedAccount.address !== lastAddressRef.current
    ) {
  ...
  return (
    <MultiViewDialog
      ...
      trigger={
        <Button
          variant="outline"
          className={["transition-[min-width] duration-300", className]
            .filter(Boolean)
            .join(" ")}
        >
          ...
          {selectedAccount?.name && (
            <span className="hidden sm:block max-w-[100px] truncate ml-1">
              {selectedAccount.name}
            </span>
          )}
          {selectedAccount?.address && (
