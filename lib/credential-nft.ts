...
import { CREDENTIAL_NFT_ADDRESS } from './config'
...
const { msg, reviveCall, sendTx } = makeInkHelpers(CREDENTIAL_NFT_ADDRESS, credentialNft)
...
export const ownerOf = async ({
  ...
  return decoded ? (u8aToHex(decoded.asBytes ? decoded.asBytes() : decoded) as `0x${string}`) : null
}
...
export const grantIssuerRole = async ({
  ...
  account: InjectedPolkadotAccount
  issuer: string
}) => sendTx(account, msg('grant_issuer_role').encode({ account: h160Binary(issuer) }))
