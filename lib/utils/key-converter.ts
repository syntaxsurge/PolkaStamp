...
import { encodeAddress, cryptoWaitReady, decodeAddress } from '@polkadot/util-crypto'
...
export async function derivePublicKeysFromPrivateKey(privateKeyHex: string): Promise<PublicKeys> {
  ...
  for (const type of types) {
    try {
      ...
      console.log(`[key-converter] ${type} candidate rejected →`, (e as Error).message)
    }
  }
...
export async function derivePublicKeysFromKeystore(
  ...
  console.log('[key-converter] derive from keystore – type:', keyType, 'address:', pair.address)
  return { ss58: pair.address, h160: toH160Hex(pair.address) }
}
